...
func worker(jobs <-chan string, out chan<- *R) {
	for filename := range jobs {
		...
		arr = append(arr, p1, p2)
		//  增加7
		p1, p2, _, _ = randomness.OverlappingTemplateMatchingProto(bits, 7)
		arr = append(arr, p1, p2)
		...
		arr = append(arr, p)
		// 增加15
		p, _ = randomness.BinaryDerivativeProto(bits, 15)
		arr = append(arr, p)
		...
		arr = append(arr, p)
		// 增加32
		p, _ = randomness.AutocorrelationProto(bits, 32)
		arr = append(arr, p)
		...
		arr = append(arr, p)
		p, _ = randomness.ApproximateEntropyProto(bits, 7)
		arr = append(arr, p)
		// 500,1000改为5000
		...
		// 13.
		p, _ = randomness.LinearComplexityProto(bits, 5000)
		arr = append(arr, p)
...
func main() {
	...
	_, _ = w.WriteString(
		"源数据," +
			...
			"扑克检测 m=4," +
			"扑克检测 m=8," +
			"重叠子序列检测 m=3 P1,重叠子序列检测 m=3 P2," +
			"重叠子序列检测 m=5 P1,重叠子序列检测 m=5 P2," +
			"重叠子序列检测 m=7 P1,重叠子序列检测 m=7 P2," +
			"游程总数检测," +
			"游程分布检测," +
			"块内最大游程检测 m=10000," +
			"二元推导检测 k=3," +
